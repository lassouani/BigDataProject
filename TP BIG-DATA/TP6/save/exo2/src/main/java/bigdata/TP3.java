...
public class TP3 {
	...
	public static class TaggedKeyWritable implements WritableComparable<TaggedKeyWritable> {
		...
		public int compareTo(TaggedKeyWritable o) {
			if(o == null) { return 1; }
			boolean result = (this.t.keyName == o.t.keyName);
			if(result) {
				return (this.t.typeKey < o.t.typeKey ? -1 : (this.t.typeKey==o.t.typeKey ? 0 : 1));
			}
	        if (this.t.keyName.hashCode() < o.t.keyName.hashCode())
				return -1;
			else
				return 1;
	  ...
	  public void reduce(Text key, Iterable<TaggedValueWritable> values, Context context) throws IOException, InterruptedException {
		  ...
		  for(TaggedValueWritable taggedvalue : values){
			if(taggedvalue.t.typeValue == 1){
				...
				context.write(NullWritable.get(), new Text(taggedvalue.t.name+","+region));
			}
		}
